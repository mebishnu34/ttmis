...
{
    $output.='
              <font size="+2"><b>प्रशिक्षकको लागि प्राप्त आवेदनहरु - तालिम विषय :-'. $_SESSION['category'] . ' शैक्षिक सत्र:-' . $_SESSION['eduyear'] .' </b></font>
   ...
   $schooldistrict="";
            ...
            $i = 1;
            if($_SESSION['category']=="एक महिने प्रमाणीकरण तालिम (TPD)")
            {
                    $oldcategory="शिक्षक पेशागत विकास";
            }
        if($_SESSION['category']=="सेवा प्रवेश तालिम")
            {
                    $oldcategory="सेवाकालिन";
            }
        if($_SESSION['category']=="कष्टमाइज (क्षमता विकास) ५ दिने")
            {
                    $oldcategory="पुनर्ताजगी";
            }
        if($_SESSION['category']=="प्रधानाध्यापक नेतृत्व सम्बन्धि कस्तमाइज तालिम")
            {
                    $oldcategory="नेतृत्व तथा व्यवस्थापन";
            }
            $sql4 = "SELECT trainingid FROM tblruntraining where trainingname='".$_SESSION['category']."' OR trainingname='".$oldcategory."'";
...
if ($result4->num_rows > 0)
   {
    while($row4 = $result4->fetch_assoc()) 
		...
		$trainingid=$row4["trainingid"];
            ...
            WHERE tt.trainingid = '".$trainingid."' and SUBSTRING(tt.sdate,1,4)='".$_SESSION['eduyear']."'
            ORDER BY t.tname
            ";
   ...
   header("Content-Type: application/xls");
	header("Content-Disposition: attachment; filename=teacher_details_category.xls");
    echo $output;
 }
?>
</BODY>
