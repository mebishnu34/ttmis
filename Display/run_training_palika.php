...
$tsql = "SELECT trainingname, level, subject, startdate, enddate,venue from tblruntraining where palikaid='$_SESSION[tid]' and trainingid='$traid'";
$tresult = $palikaconn->query($tsql);
...
<?php
$sn=1;
$sql1 = "SELECT ID,teacherid,schoolcode,sdate,edate FROM tblttraining where munid='$_SESSION[tid]' and trainingid='$traid' and remark='Running'";
...
if ($result->num_rows > 0)
   {
	...
	$scode=$row["schoolcode"];
		$sqlt = "SELECT tname,tcontact FROM tblteacher where teachercode='$tcode' or teacherid='$tcode'";
