...
=======
	if($rownum->num_rows>0)
		...
		{
			$sql="Insert into tblmember(fname,gender,memphoto,level,faculty,institute, email, contact, district, l_govern, wardno, address, memtype,lname,lpass,ondate,remark) 
				values('$uname','None','','','','','$email','$mobile','$district','$lgov','$ward','$address','$utype','$mobile','$mobile',now(),'Active User')";
			if(!mysqli_query($conn,$sql))
				{
					header('Location: ../new_user.php?msg=' . mysqli_error($conn));
					//echo mysqli_error($conn);
