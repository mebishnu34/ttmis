...
if(!in_array($extension, ['mp4', 'avi','mp3'])) 
	{
    header('Location: ../Admin/artical/video_1.php?msg="Your file extension must be .mp4, or .avi"');
     //echo "Your file extension must be .mp3, or .wav";
    } 
elseif ($_FILES['videofile']['size'] > 1000000) // file shouldn't be larger than 1Megabyte
	{ 
		header('Location: ../Admin/artical/video_1.php?msg="File too large"');
       // echo "File too large!";
    } 
else
  {
	...
	if ($result->num_rows > 0)
   		{
			header('Location: ../Admin/artical/video_1.php?msg="Found Duplicate"');
		}
	else
		{
	     if (move_uploaded_file($file, $destination))     // move the uploaded (temporary) file to the specified destination
         	...
         	if (mysqli_query($conn, $sql)) 
		 		{
                	header('Location: ../Admin/artical/video_1.php?msg="Save Successfully"');
            	}
			else
				{
					 header('Location: ../Admin/artical/video_1.php?msg'. mysqli_error($conn));
				//echo mysqli_error($conn);
				}
        	} 
		else
		 {
           header('Location: ../Admin/artical/video_1.php?msg="Failed to upload file"');
