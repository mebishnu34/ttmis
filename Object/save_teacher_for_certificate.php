          ...
          if($result1->num_rows > 0)
            {
              if($row1 = $result1->fetch_assoc())
                  {
                      mysqli_query($conn,"INSERT INTO tblttraining(teacherid, trainingid,runid, schoolcode,munid,teacherdistrict ,gnumber, coordinator, mobileno, sdate, edate, remark,certificate,certificatenumber,registernumber,prepairedby, checkby,approvedby) values('".$_POST['txtcode']."','0','".$trainingrunid."','$scode','0','', '1', '".$row1['coordinator']."','".$row1['mobileno']."','".$row1['startdate']."','".$row1['enddate']."','completed','','','','','','')");
