	...
	if($results->num_rows>0)
		{
			 ...
			 $sql = "UPDATE tblteacher set tname='$tname', gender='$gender',cast='$cast',mothertong='$mothertong',citizenship='$citizen',sheetroll='$rollno',stream='$stream',fathername='$fathername', dob='$dob', address='$a', email='$email', wardno='$ward',tcontact='$tc', appointdate='$adate', appointtype='$atype',workinglevel='$level',subject='$apsubject', scode='$scode',category='Teacher',teachinglevel='$tlevel', qualification='$qualification', faculty='$faculty', majorsubject='$msubject', teachingsubject='$tsubject',loginname='$loginname',tpass='$upass' where teacherid='$tid'";
             if (mysqli_query($conn, $sql))
              {
              header('Location: ../application.php?msg= "Update Successfully"');
              }
              else
              {
		...
		{
		header('Location: ../application.php?msg="Invalid School Code"');
		}
  }
  else
  {
    header('Location: ../application.php?msg="Password Not Match"');
  }
}
mysqli_close($conn);
?>
