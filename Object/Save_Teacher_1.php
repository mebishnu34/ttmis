 ...
 $result = $conn->query($sql1);
         if ($result->num_rows > 0)
         {
		  ...
		  header('Location: ../municipality_application.php?msg= "Sorry,Try Later..."');
            ...
            {
             $sql = "INSERT INTO tblteacher(teachercode,tname, gender,cast,mothertong,citizenship, sheetroll,subject,loginname, tpass,dob,fathername, address, email,district, munvdc, wardno,tcontact, appointdate, appointtype,workinglevel, scode,schoolname,schooladdress,province,category, teachinglevel, qualification, faculty, majorsubject, teachingsubject, remark, importno,username,teachertype,stream,trank,position,contact) values('$tid','$tname', '$gender','$cast','$tong','$citizen','$rollno','$apsubject','$tid','$tid','$dob','$fname','$a','$email','$d','$vdc', '$ward','$tc', '$adate','$atype','$level','$scode','$schoolname','','','Teacher','$tlevel', '$qualification', '$faculty', '$msubject', '$tsubject','Working','0','','','','','','')";
             if (mysqli_query($conn, $sql))
              {
			  $message="TTMIS::Login Name:-".$ln." and Password:-".$tid;
		 	  $mobileno=$tc;
