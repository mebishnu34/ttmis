    	...
    	while($row1 = $result1->fetch_assoc())
     		{
				...
				if($tcode<>"")
        		{
                	...
                	if ($result->num_rows > 0)
                	{
                	header('Location: ../Input/student_search_by_mobileno.php?msg="Already Exist"');
                 	}
    	          	else
                	{
                		$sql = "INSERT INTO tblttraining(teacherid, trainingid,runid, schoolcode,munid,teacherdistrict ,gnumber, coordinator, mobileno, sdate, edate, remark,certificate,certificatenumber,registernumber,prepairedby, checkby,approvedby) values('$tcode','$trainingid','$rnid','$scode','$munid','', '$gn', '$coordinator','$mobileno','$sdate','$edate','Running','','','','','','')";
                		...
                		header('Location: ../Input/student_search_by_mobileno.php?msg= "Saved Successfully"');
                		}
                		else
                		{
                		$message= "Save_teacher_participate_in_training" . $sql . "<br>" . mysqli_error($conn);
		  				...
		  				header('Location: ../Input/student_search_by_mobileno.php?msg= "Error"');
                		}
                	}
              	}
          		else
          		{
           		header('Location: ../Input/student_search_by_mobileno.php?msg= "Fields Are Required"'); 
          		}	
        	}
